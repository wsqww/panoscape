...
10. **切底图相机楔入山体（黑屏）**：开启地形时在**高山特写相机**（如武功山 pitch 62、zoom 15）下 `setStyle` 切底图，新样式 DEM 异步重载期间相机约束按平地求解，相机会被楔进山体——症状为画布全黑（含 DOM 标记一起「消失」的观感）、`project()` 返回画面外坐标，但 `isStyleLoaded/getLayer` 全部正常且零报错；DEM 就绪后不会自愈。已在 `switchBaseLayer` 修复：`style.load` 重挂图层后 `once('idle')` 用当前相机值重发一次 `jumpTo`，强制约束重新求解（同值 jumpTo 对平缓场景无副作用）。排查此类黑屏先 `project()` 检查相机是否错位，勿盲目怀疑瓦片网络。**同类问题在飞行落地时也会出现**：flyTo 动画期间约束按不完整 DEM 持续求解，落地点被推离目标（目标 `project()` 偏离屏幕中心数百 px）。已在 `flyToAttraction`（tour.ts）修复：① 景区可配 `twoPhaseFlight`——两段式飞行（先高位 pitch0 推近目标区域触发高程加载，再降到取景参数），从源头减少约束推挤；② 落地后 moveend/idle/400ms 定时器三路检查偏移（阈值 80px）与 zoom 被压低（>0.3），用 jumpTo 瞬时重申（easeTo/flyTo 动画修正会被约束逐帧压制无法收敛）+ 画布 transform 反向平移 700ms 缓出滑回正中并恢复取景缩放；③ 全程带交互守卫（用户拖动/缩放即取消修正）与飞行序号（仅最新飞行生效）。注意修正等 DEM 就绪后才执行（queryTerrainElevation 可查询），过早缓动会被约束逐帧压回
